                                        ...
                                        <div class="profile-colnt">
                                            <div class="profile-colnt-det">
                                                ...
                                                @if (!empty($case->doctor_id))
                                                    <h3> {{ $case->doctor->name }} <mdall></mdall>
                                                    </h3>
                                                @endif
                        ...
                        <div class="col-md-8">
                            <div class="chat-body" id="chat_list" style="height: 300px;">
                                ...
                                <div class="chat-body-me">
                                    <div class="chat-body-left">
                                        <img src="{{ Auth::guard('sitePatient')->user()->profile->profile_photo }}"
                                            alt="">
                                    </div>
                                    <div class="chat-body-right">
                                        <h3>You <mdall>{{ date('d-m-Y h:i:s', strtotime($case->created_at)) }}</mdall>
                                        </h3>
                                    ...
                                    <div class="chat-body-me">
                                        ...
                                        <div class="chat-body-right">
                                            ...
                                            <img class="img-thumbnail mb-3"
                                                src="{{ asset('public/uploads/cases/' . $case->casefile->file_name) }}">
                                        </div>
                                    </div>
                                @endif
                            ...
                            <form method="post" id="upload_form" enctype="multipart/form-data">
                                ...
                                <input type="hidden" name="rec_user_id" id="rec_user_id"
                                    value="{{ $case->doctor_id ?? '' }}" data-patient-count="">
                                ...
                                <button type="submit"><img
                                        src="{{ asset('public/images/frontend/images/chat-icon.png') }}" alt=""></button>
                            </form>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        setTimeout(() => {
            ...
            db.collection("chats").where("case_id", "==", '{{ $case->case_id }}').onSnapshot(function(
                querySnapshot) {
                querySnapshot.forEach((doc) => {
                    var details = doc.data();
                    console.log(details);
            ...
            setTimeout(() => {
                ...
                $.each(chatDetails, function(key, value) {
                    ...
                    if (value.sender_id != '{{ Auth::guard('sitePatient')->user()->id }}') {
                        chatData +=
                            '<div class="chat-body-you"><div class="chat-body-left"><img src="{{ $case->doctor->profile->profile_photo }}" alt=""></div><div class="chat-body-right"><h3>{{ $case->doctor->name }}  <mdall>' +
                            time + '</mdall></h3><p>' + value.message + '</p></div></div>';

                    } else {
                        pat_count++;
                        chatData +=
                            '<div class="chat-body-me"><div class="chat-body-left"><img src="{{ Auth::guard('sitePatient')->user()->profile->profile_photo }}" alt=""></div><div class="chat-body-right"><h3>You <mdall>' +
                            time + '</mdall></h3><p>' + value.message + '</p></div></div>';
                    }
                });

                // $(".overlay").hide();
                $("#rec_user_id").data('patient-count', pat_count);
        ...
        $('#upload_form').on('submit', function(event) {
            ...
            if($('#rec_user_id').data('patient-count') <3) {
                if ($('#image').val()) {
                ...
                Swal.fire(
                'Warning!',
                'You can not enter more than three message!',
                'warning'
                );
            }
        });
