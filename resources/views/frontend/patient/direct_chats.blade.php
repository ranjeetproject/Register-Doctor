@extends('frontend.patient.afterloginlayout.apps')
            ...
            setTimeout(() => {
                ...
                $.each(chatDetails, function(key, value) {
                    ...
                    if (value.sender_id != '{{ Auth::guard('sitePharmacist')->user()->id }}') {
                        ...
                        chatData +=
                            '<div class="chat-body-me"><div class="chat-body-left"><img src="{{ Auth::guard('sitePharmacist')->user()->profile->profile_photo }}" alt=""></div><div class="chat-body-right"><h3>You <small>' +
                            time + '</small></h3><p>' + value.message + '</p></div></div>';
                        doc_count++;
                    }
                });
        ...
        $('#upload_form').on('submit', function(event) {
            ...
            // if ($('#rec_user_id').data('doctor-count') < 10) {
                ...
                if ($('#image').val()) {
                    ...
                    var currentTime = d.getTime();
                    db.collection("chats").add({
                            sender_id: '{{ Auth::guard('sitePharmacist')->user()->id }}',
