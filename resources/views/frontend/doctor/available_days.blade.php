        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <ul class="nav Calendar-Regular-tab" id="myTab" role="tablist">
                                    ...
                                    <li class="nav-item">
                                      <a class="tips"><img src="{{ asset('public/images/frontend/images/ex-icon.png') }}" alt="" data-toggle="tooltip" data-placement="right" title="" data-original-title="One line definition"> How It Works</a>
                                    </li>
                                  </ul>
...
<div class="col-sm-12">
    <table border="0"> 
        ...
        <tbody>
            ...
            <tr>
                <td>{{date('l',strtotime($available_day->date)) .' - '. date('M d Y',strtotime($available_day->date))}}</td>
                <td>{{date('H:i a', strtotime($available_day->from_time))}}</td>
                <td>{{date('H:i a', strtotime($available_day->to_time))}}</td>
                <td style="text-align: center;"><a href="#" onclick="editAvailableDay('{{$available_day->id}}');"><i class="fas fa-pencil-alt"></i></a> | <a href="{{route('doctor.delete-available-day',$available_day->id)}}" onclick="return confirm('Are you sure ?');"><i class="far fa-trash-alt"></i></a></td>
            </tr>
             @empty
              <tr>
    ...
    <div class="modal fade bd-example-modal-lg how-it-works" tabindex="-1" id="myModal" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="fal fa-times-circle"></i>
              </button>
            <p>Under Regular Weekly Timetable input or delete any regular slots you offer - This will show as your weekly timetable indefinitely e.g. Sundays 09:00 AM to 10:00 AM and Mondays 06:00 PM to 07:15 PM.</p>
            <p>Under Ad Hoc Timeslots add non-regular Timeslots or delete specific dates.
                e.g. add next Tuesday 09-10 PM as you will be available, or cancel all slots in May as you are away. If you use only Ad Hoc Timeslots remember to keep updating your availability.</p>
        </div>
        </div>
      </div>
      ...
      <div class="modal fade bd-example-modal-lg how-it-works" tabindex="-1" id="myModal2" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="fal fa-times-circle"></i>
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <form method="POST" action="{{route('doctor.edit-available-day')}}">
                ...
                <div class="form-group">
                    <input type="text" onfocus="(this.type='date')" name="date" id="edit_date" class="form-control" required>
                </div>
      ...
      <div class="modal fade bd-example-modal-lg how-it-works" tabindex="-1" id="myModal5" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="fal fa-times-circle"></i>
  ...
  // window.location.reload(true)
      ...
      function editAvailableDay(available_day_id) {
             ...
             $.ajax({
                ...
                dataType: "json",
                // data:{state:state,type:type,_token:token}
                data:{ available_day_id:available_day_id}
                }).done(function(response) {
                    if(typeof response != "undefined" && response.success){
                    $('#myModal3').modal('show');
      ...
      function addweeklyday() {
       ...
       function editWeeklyDay(weekly_day_id) {
             ...
             $.ajax({
                ...
                dataType: "json",
                // data:{state:state,type:type,_token:token}
                data:{ weekly_day_id:weekly_day_id}
                }).done(function(response) {
                    if(typeof response != "undefined" && response.success){
                    $('#myModal5').modal('show');
