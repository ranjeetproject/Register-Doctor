                        ...
                        <div class="col-sm-8">
                             ...
                             <div class="chat-body-you">
                                    ...
                                    <div class="chat-body-right">
                                        <h3>{{$case->user->name}}  <small>{{date('d-m-Y h:i:s',strtotime($case->created_at))}}</small></h3>
                           ...
                           @csrf
                              <input type="hidden" name="rec_user_id" id="rec_user_id" value="{{$case->user_id ?? ''}}">
...
$.each(chatDetails, function (key, value) {
  var time = moment(value.created_at).format("DD-MM-YYYY h:mm:ss");
      if(value.sender_id != '{{Auth::guard('siteDoctor')->user()->id}}' ){
         chatData += '<div class="chat-body-you"><div class="chat-body-left"><img src="{{$case->user->profile->profile_photo}}" alt=""></div><div class="chat-body-right"><h3>{{$case->user->name}}  <small>'+time+'</small></h3><p>'+value.message+'</p></div></div>';
        
      }else{
            chatData += '<div class="chat-body-me"><div class="chat-body-left"><img src="{{ Auth::guard('siteDoctor')->user()->profile->profile_photo }}" alt=""></div><div class="chat-body-right"><h3>You <small>'+time+'</small></h3><p>'+value.message+'</p></div></div>';
      }
  });
// $(".overlay").hide();
...
$.ajax({
  ...
  var currentTime = d.getTime();
    db.collection("chats").add({
    sender_id: '{{ Auth::guard('siteDoctor')->user()->id }}',
