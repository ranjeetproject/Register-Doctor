...
<div class="card card-primary card-outline">
    <div class="card-header">
        <h3 class="card-title">Handy Document list</h3>
    ...
    <div class="card-body p-0">
        <form action="{{ route('admin.h_doc.delete') }}" method="post">
            @csrf
                            ...
                            <tr onmouseover="showActionBtn('{{ $loop->iteration }}');"
                                ...
                                <td>
                                    <div class="icheck-primary">
                                        <input type="checkbox" name="h_doc_id[]" value="{{ $h_doc->id }}"
                                            id="check{{ $loop->iteration }}">
                                        <label for="check{{ $loop->iteration }}">#{{ $loop->iteration }}</label>
                                    </div>
                                </td>
                                <td>{{ $h_doc->topic_name }}</td>
                                <td>
                                    @switch($h_doc->user_role)
                                        @case(1)
                                            Patients
                                        @break
                                        @case(2)
                                ...
                                <td>{{ date('M-d-Y H:i:s', strtotime($h_doc->created_at)) }}</td>
                                <td>
                                    {{-- <a class="btn btn-sm btn-outline-primary" href="{{ route('admin.h_doc', $h_doc->id) }}"> <i class="fas fa-eye"></i> View</a> --}}
                                    <a class="btn btn-sm btn-outline-warning"
                                        href="{{ route('admin.h_doc.edit', $h_doc->id) }}"> <i
                                            class="fas fa-edit"></i> Edit</a>
                                    <a class="btn btn-sm btn-outline-danger"
                                        onclick="return confirm('Are you sure want to delete?');"
                                        href="{{ route('admin.h_doc.delete', $h_doc->id) }}"> <i
