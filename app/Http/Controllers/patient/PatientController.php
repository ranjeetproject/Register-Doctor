...
// dd($request->all());
   ...
   public function showPrescriptionsRules(Request $request)
    ...
    {
        $cases = PatientCase::orderBy('created_at', 'DESC')->where('user_id',Auth::guard('sitePatient')->user()->id)->where('accept_status',1)->get();
    ...
    public function pharmacies(Request $request)
    {
        ...
        $pharmacies = User::with(["specialAvailabilities" => function($q){
            $q->whereDate('special_availabilities.available_at', '>=', date('Y-m-d'));
        }])->whereRole(3)->when($search, function ($query) use ($search) {
