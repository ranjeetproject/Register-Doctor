...
class CoronavirusController extends Controller
{
    ...
    {
    	$news = Coronavirus::latest()->get();
    	return view('admin.coronavirus.index', compact('news'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->hasFile('image')){
            ...
            $destinationPath    = public_path().'/uploads/corona';
    	...
    	return view('admin.coronavirus.create');
    }

    public function edit(Request $request,$id)
    {
        $news = Coronavirus::find($id);
        if ($request->isMethod('post')) {
            $validator = $request->validate(
           [
              "heading"=>"required",
        ...
        return view('admin.coronavirus.edit', compact('news'));
    }
    ...
    public function delete(Request $request, $nid = '')
    {
      if ($request->isMethod('post')) {
            foreach ($request->news_id as $id) {
                ...
                $news = Coronavirus::find($nid);
                // $news->profile->delete();
                $news->delete();
      }
       Session::flash('Success-toastr', 'Successfully deleted.');
       return redirect()->route('admin.coronavirus');
    }

    public function slideSelectRemove($id, $status)
    {
        if($status == 1){
            $five_count = Coronavirus::where('slide_status', 1)->count();
            if($five_count >= 5){
